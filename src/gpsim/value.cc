...
{
    // FIXME - casting a signed int to an uint -- probably should issue a
    // warning
    int64_t i64;
    get(i64);
    i = (int64_t)i64;
...
{
    throw new Error(showType() + " cannot be converted to a boolean");
}

void Value::get(double &d)
{
    throw new Error(showType() + " cannot be converted to a double ");
}
...
//------------------------------------------------------------------------
ValueWrapper::ValueWrapper(Value *pCopy) : m_pVal(pCopy) {}
...
Boolean *Boolean::NewObject(const char *_name, const char *pValue,
                            const char *desc)
{
    bool bValue;
 ...
 * The Integer class.
 */
Integer::Integer(const Integer &new_value) : Value()
{
...
bool Integer::Parse(const char *pValue, int64_t &iValue)
{
    if (::isdigit(*pValue)) {
        if (strchr(pValue, '.'))
            ...
            return sscanf(pValue, "%li", &iValue) == 1;
...
Integer *Integer::assertValid(Value *val, string valDesc, int64_t valMin)
{
    ...
    if (i < valMin) {
        throw new Error(valDesc + " must be greater than " +
                        Integer::toString(valMin) + ", saw " +
                        Integer::toString(i));
    }
                              ...
                              int64_t valMax)
{
    ...
    if ((i < valMin) || (i > valMax)) {
        throw new Error(valDesc + " must be be in the range [" +
                        Integer::toString(valMin) + ".." +
                        Integer::toString(valMax) + "], saw " +
                        Integer::toString(i));
    }
...
void Float::set(Value *v)
{
    ...
    */
    ...
    if (typeid(*v) != typeid(Float) && typeid(*v) != typeid(Integer)) {
