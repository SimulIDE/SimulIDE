...
{
    // qDebug() <<"eSource::stampOutput"<<QString::fromStdString(m_elmId)<<
    // m_out<<m_voltOut<<m_imp ;
...
void eSource::setOut(bool out) // Set Output to Hight or Low
{
    ...
    // qDebug() <<
    // "eSource::setOut"<<QString::fromStdString(m_elmId)<<m_voltOut;
}
bool eSource::out()
{
    return m_out;
...
{
    // qDebug() << "eSource::getVolt()"<<QString::fromStdString(m_elmId)<<
    // m_ePin[0]->isConnected() <<m_voltOut;
