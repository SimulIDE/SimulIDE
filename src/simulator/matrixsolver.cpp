...
void MatrixSolver::simplifyMatrix(vector<vector<double>> circuitMatrix,
                                  std::vector<double> circuitRightSide)
{
    ...
    for (int i = 0; i != matrixSize; i++) {
        ...
        for (j = 0; j != matrixSize; j++) {
            ...
            // std::cout<< "Row "<< i << "col" <<j<< " element = " << q
            // <<std::endl;
            if (q == 0)
                continue;
        ...
        if (j == matrixSize) // 2 or less Non Cero
        {
            ...
            if (qm == -1) // 1 Non Cero
            {
                ...
                // is a constant
                for (int k = 0; elt.type == RowInfo::ROW_EQUAL && k < 100;
                     k++) {
                    // follow the chain
