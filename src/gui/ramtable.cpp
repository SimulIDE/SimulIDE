...
RamTable::RamTable(BaseProcessor *processor) : QTableWidget(60, 4)
{
    ...
    connect(this, SIGNAL(itemChanged(QTableWidgetItem *)), this,
            SLOT(addToWatch(QTableWidgetItem *)));
...
void RamTable::slotContextMenu(const QPoint &point)
{
    ...
    if (m_debugger) {
        QAction *loadVars =
            menu.addAction(QIcon(":/open.png"), tr("Load Variables"));
        connect(loadVars, SIGNAL(triggered()), this, SLOT(loadVariables()));
    }

    QAction *clearSelected =
        menu.addAction(QIcon(":/remove.png"), tr("Clear Selected"));
    connect(clearSelected, SIGNAL(triggered()), this, SLOT(clearSelected()));

    QAction *clearTable =
        menu.addAction(QIcon(":/remove.png"), tr("Clear Table"));
    ...
    QAction *saveVarSet =
        menu.addAction(QIcon(":/save.png"), tr("Save VarSet"));
...
void RamTable::loadVarSet()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(
        this, tr("Load VarSet"), dir, tr("VarSets (*.vst);;All files (*.*)"));
...
void RamTable::saveVarSet()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Save VarSet"), dir, tr("VarSets (*.vst);;All files (*.*)"));
    if (!fileName.isEmpty()) {
...
void RamTable::updateValues()
{
    if (m_processor) {
        for (int _row : watchList.keys()) {
            ...
            if (!ok)
                m_processor->updateRamValue(name); // Var or Reg name
            else                                   // Address
            {
...
void RamTable::addToWatch(QTableWidgetItem *it)
{
    ...
    QString name =
        it->text().remove(" ").remove("\t").remove("*"); //.toLower();
